...
if [ $TARGET = "windows" ]; then
    if [ `ls -A $LIBDIR | wc -w` == 0 ]; then
        wget -q https://bitbucket.org/rude/love/downloads/love-11.3-win64.zip -O $ROOTDIR/temp/windows.zip
    ...
    if [ `ls -A $LIBDIR | wc -w` == 0 ]; then
        wget -q https://bitbucket.org/rude/love/downloads/love-11.3-macos.zip -O $ROOTDIR/temp/osx.zip
        unzip -q $TEMPDIR -d $LIBDIR/
